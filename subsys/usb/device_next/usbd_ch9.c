...
static int usbd_enqueue_status_in(struct usbd_context *const uds_ctx)
{
	...
	status_in = usbd_alloc_status_stage(uds_ctx, USB_CONTROL_EP_IN);
	if (status_in == NULL) {
		return -ENOMEM;
	}
...
static int usbd_enqueue_status_out(struct usbd_context *const uds_ctx)
{
	...
	if (caps.autostatus_after_data_in) {
		/* Nothing to do, controller will handle things automatically */
		return 0;
	}

	status_out = usbd_alloc_status_stage(uds_ctx, USB_CONTROL_EP_OUT);
	if (status_out == NULL) {
		return -ENOMEM;
	}
