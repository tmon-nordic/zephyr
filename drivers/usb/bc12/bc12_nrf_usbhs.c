					  ...
					  const enum bc12_type type)
{
	...
	if (data->state == USBHS_BC_FINISHED && data->result_cb != NULL) {
...
static void usbhs_bc_start_primary_detection(const struct device *dev)
{
	...
	wrapper->PHY.OVERRIDEVALUES = BIT(USBHS_PHY_OVERRIDEVALUES_OPMODE0_Pos) |
				      BIT(USBHS_PHY_OVERRIDEVALUES_XCVRSEL0_Pos);
...
static void usbhs_bc_check_secondary_detection(const struct device *dev)
{
	...
	nrf_usbhs_wrapper_bc12_finished(config->parent, type == BC12_TYPE_DCP);
}
...
static void usbhs_bc_check_primary_detection(const struct device *dev)
{
	...
	if (need_secondary_detection) {
		...
		usbhs_bc_finish_state(dev);
		usbhs_bc_set_detection_result(dev, type);
		nrf_usbhs_wrapper_bc12_finished(config->parent, false);
	}
}
...
static void usbhs_bc_timer_expiry(struct k_timer *timer)
{
	...
	/*
	 * If the VBUS removed event occurs at the same time as the timer is
	 * finished, do nothing because the timer will start with a new period.
	 */
	if (!data->vbus_present && data->state != USBHS_BC_DISCONNECTED) {
		LOG_DBG("VBUS removed as timer expired");
...
static void usbhs_bc_vbus_detected(const struct device *dev)
{
	...
	nrf_usbhs_wrapper_bc12_vbus_detected(config->parent);
...
static void usbhs_bc_vbus_removed(const struct device *dev)
{
	...
	nrf_usbhs_wrapper_bc12_vbus_removed(config->parent);
...
static int usbhs_bc_set_role(const struct device *dev, const enum bc12_role role)
{
	...
	if (data->state != USBHS_BC_FINISHED && data->state != USBHS_BC_WAIT_FOR_VBUS) {
		LOG_ERR("BC detection is in progress");
		err = -EBUSY;
		goto error;
	}

	if (data->partner_state.bc12_role == role) {
		err = -EALREADY;
		goto error;
	}
	...
	if (role == BC12_DISCONNECTED) {
		...
		nrf_usbhs_wrapper_bc12_pd_enabled(config->parent, false);
	}
	...
	if (role == BC12_PORTABLE_DEVICE) {
		...
		data->partner_state.type = BC12_TYPE_NONE;
		data->partner_state.bc12_role = role;
		nrf_usbhs_wrapper_bc12_pd_enabled(config->parent, true);
		err = nrf_usbhs_wrapper_vreg_enable(config->parent);
		if (err) {
			nrf_usbhs_wrapper_bc12_pd_enabled(config->parent, false);
			LOG_ERR("Failed to enable regulator %s error %d",
				config->parent->name, err);
			goto error;
		}
	}
...
static int usbhs_bc_init(const struct device *dev)
{
	...
	nrf_usbhs_wrapper_set_bc12_cb(config->parent, vregusb_event_cb, dev);
